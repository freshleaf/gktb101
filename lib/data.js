...
exports.getUniversityMajorScore = function (universityCode, studentLocCode, year) {
    var sql = 'select major_line.year, code, name, major_line.student_type, major_line.batch, max_score, min_score, average_score, admissions_number' +
        ' from major_line' +
        ' left join' +
        ' (select subclass_code as code, subclass_name as name from major_subclass' +
        ' union' +
        ' select major_code as code, major_name as name from major) as major_info' +
        ' on major_line.major_code = major_info.code' +
        ' where registration_location_code = "' + studentLocCode +'"' +
        ' and university_code = ' + universityCode +
        ' and major_line.year = ' + year +
        ' order by year desc, student_type, average_score desc, min_score desc, max_score desc';
    const results = connection.query(sql);
    return results;
};